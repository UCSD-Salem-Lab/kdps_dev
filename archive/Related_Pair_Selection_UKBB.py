...
def get_Related(all_samples,kinship_file,kinship_threshold):
    ...
    # Load kinship file
    kinship = pd.read_csv(kinship_file,header=None,names=["ID1","ID2","KINSHIP"],delimiter=" ", converters = {'KINSHIP': float,'ID1': str,'ID2': str})
    # Filter kinship file so that pairs that are remaining are all in sample file
    kinship2 = kinship[kinship.ID1.isin(all_samples.IID)]
    kinship3 = kinship2[kinship2.ID2.isin(all_samples.IID)]
    # Filter out by kinship threshold
    kinship4 = kinship3[kinship3["KINSHIP"] >= kinship_threshold ]
    # Filter the Kinship column out
    kinship5 = kinship4[["ID1", "ID2"]]
...
#
# Parameters (2) : 1. related_people (pd data frame) : dataframe created with two columns: ID1, ID2
...
def get_Strict_Unrelated(related_people,all_samples):
    ...
    # Load sample file
    unrelated = all_samples[~all_samples.IID.isin(related_people.ID1)] 
    unrelated2 = unrelated[~unrelated.IID.isin(related_people.ID2)] 
...
#                  2. related people (pd data frame) : data frame created with two columns: ID1,ID2
...
def get_Pheno(pheno_file,related_people,all_samples):
    ...
    first = related_people['ID1'].to_numpy()
    second = related_people['ID2'].to_numpy()
...
def calculate_score(ID,phenotype_dict,relatedness_df):
    ...
    ID=str(ID)
    x=relatedness_df[relatedness_df.ID1 == ID]
    l1 = x["ID2"].tolist()
    x=relatedness_df[relatedness_df.ID2 == ID]
    l2 = x["ID1"].tolist()
    list_related=l1+l2
...
def main(args):
    ...
    # Generate a Table of the related People. Output a pandas dataframe with two columns ID1 and ID2 in which individuals are related 
    # to each other. 
    # dtype str 
    ...
    for i, k in enumerate(new_score):
        if (new_score[k]==0):
            ...
            ID=str(k)
            x=related[related.ID1 == ID]
            l1 = x["ID2"].tolist()
            x=related[related.ID2 == ID]
            l2 = x["ID1"].tolist()
            list_related=l1+l2
            ...
            index = related[ related['ID1'] == k ].index
            related.drop(index , inplace=True)
            index = related[ related['ID2'] == k ].index
            related.drop(index , inplace=True)
    ...
    while(len(list_cases)>0):
        ...
        index = related[ related['ID1'] == max_k ].index
        related.drop(index , inplace=True)
        index = related[ related['ID2'] == max_k ].index
        related.drop(index , inplace=True)
